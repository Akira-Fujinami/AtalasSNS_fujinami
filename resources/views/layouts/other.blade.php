        ...
        @foreach ($others as $others)
            @if($others->images == "dawn.png")
                <div class="others-icon">
                    <img src="/images/icon1.png" alt="その他ユーザーのアイコン">
                </div>
            @else
                <div>
                    <img src="{{asset('storage/'.$others->images)}}" alt="その他ユーザーのアイコン" class="others-image">
                </div>
            @endif
            <div class="others">
            ...
            <div class="other-follow">
                @if (Auth::user()->isFollowing($others->id))
                    {{Form::open(['url'=>'others/unfollow/{id}'])}}
                        ...
                        <td>{{Form::submit('フォロー解除',['class'=>'btn-unfollows'])}}
                        </td>
                    {{Form::close()}}
                @else
                    {{Form::open(['url'=>'others/follow/{id}'])}}
                        {{Form::hidden('id',$others->id)}}
                        <td>{{Form::submit('フォローをする',['class'=>'btn-follows'])}}
                        </td>
                    {{Form::close()}}
                @endif
            </div>
